...
namespace WCYDataSource
{
    ...
    public class Hid : object
    {
        ...
        private const int MAX_USB_DEVICES = 164;
        public bool deviceOpened = false;
        //private bool deviceOpened2 = false;
        //private bool deviceOpened3 = false;
        ...
        static IntPtr device1;
        static IntPtr device2;
        static IntPtr device3;
        static IntPtr device4;
        static IntPtr device5;
        static IntPtr device6;
        static IntPtr device7;
        static IntPtr device8;


        public HID_RETURN OpenDevice_1(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                ...
                {
                    device1 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device1 != INVALID_HANDLE_VALUE)
                    {
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device1, out attributes);
                        HidD_GetSerialNumberString(device1, serialBuff, 512);
                        ...
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device1, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device1, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
                            //CloseHandle(device1);
        ...
        public HID_RETURN OpenDevice_2(UInt16 vID, UInt16 pID, string serial)
        {
            //CloseHandle(device1);
            if (deviceOpened == false)
            {
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device2 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device2 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device2, out attributes);
                        HidD_GetSerialNumberString(device2, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device2, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device2, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
                            //CloseHandle(device2);
        ...
        public HID_RETURN OpenDevice_3(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device3 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device3 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device3, out attributes);
                        HidD_GetSerialNumberString(device3, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device3, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device3, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
        ...
        public HID_RETURN OpenDevice_4(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device4 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device4 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device4, out attributes);
                        HidD_GetSerialNumberString(device4, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device4, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device4, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
        ...
        public HID_RETURN OpenDevice_5(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device5 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device5 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device5, out attributes);
                        HidD_GetSerialNumberString(device5, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device5, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device5, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
        ...
        public HID_RETURN OpenDevice_6(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device6 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device6 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device6, out attributes);
                        HidD_GetSerialNumberString(device6, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device6, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device6, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
        ...
        public HID_RETURN OpenDevice_7(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device7 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device7 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device7, out attributes);
                        HidD_GetSerialNumberString(device7, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device7, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device7, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
        ...
        public HID_RETURN OpenDevice_8(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    device8 = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    if (device8 != INVALID_HANDLE_VALUE)
                    {
                        //index++;
                        HIDD_ATTRIBUTES attributes;
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetAttributes(device8, out attributes);
                        HidD_GetSerialNumberString(device8, serialBuff, 512);
                        string deviceStr = Marshal.PtrToStringAuto(serialBuff);
                        Marshal.FreeHGlobal(serialBuff);
                        if (attributes.VendorID == vID && attributes.ProductID == pID)// && deviceStr == serial)
                        {
                            ...
                            IntPtr preparseData;
                            HIDP_CAPS caps;
                            HidD_GetPreparsedData(device8, out preparseData);
                            HidP_GetCaps(preparseData, out caps);
                            HidD_FreePreparsedData(preparseData);
                            outputReportLength = caps.OutputReportByteLength;
                            inputReportLength = caps.InputReportByteLength;

                            hidDevice = new FileStream(new SafeFileHandle(device8, false), FileAccess.ReadWrite, inputReportLength, true);
                            deviceOpened = true;
                            BeginAsyncRead();
        ...
        /// </summary>
        /// 
        public static bool IsCloseObject = false;
        public void CloseDevice1()
        {
            try
            {
        ...
        public static bool IsPortChange()
        {
            try
            {
                ...
                if (DeviceCount != deviceList.Count)
                {
                    DeviceCount = deviceList.Count;
                    CloseHandle(device1);
                    CloseHandle(device2);
        ...
        private void ReadCompleted(IAsyncResult iResult)
        {
            ...
            try
            {
                ...
                report e = new report(readBuff[0], readBuff.Skip(1).Take(readBuff.Length - 1).ToArray());
                OnDataReceived(e); //发出数据到达消息
                BeginAsyncRead();//启动下一次读操作
            }
            catch (Exception exc) //读写错误,设备已经被移除
            {
        ...
        public HID_RETURN Write(report r)
        {
            if (deviceOpened)
            {
                try
                {
                    ...
                    //for (int i = maxBufferLength; i < outputReportLength; i++)
                    //    buffer[i] = 0x00;
        ...
        {
            report r = new report(0x00, sendBuffer);
            Write(r);
        }
