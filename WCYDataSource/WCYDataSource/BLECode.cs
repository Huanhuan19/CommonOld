...
namespace WCYDataSource
{
    public class BluetoothLECode
    {
        ...
        private void DeviceWatcher_Added(DeviceWatcher sender, DeviceInformation args)
        {
            ...
            switch (args.Name.Substring(0, 2))
            {
                case "A5":
                case "B5":
                case "C5":
        ...
        private void Characteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            lock (readList)
            {
                ...
                //TimeSpan tf = DateTime.Now - tt;
                //if (tf.TotalSeconds >=100)
                //{
                //    int count = (int)(readList.Count / 51);
                //    count = count * 16;
                //}
                if (CheckList(readList))
                {
                    if (readList.Count >= 51&& (readList[0]) == 0xa5)
                    {

                        var value = readList.Take(50).ToArray();
                        readList = readList.Skip(51).ToList();
                        ValueChanged(MsgType.BLEData, value);
                    }
                    else if (readList[0] == 0x5a || (readList[1]) == 0xa5)
                    {
                        int len = 5 + readList[4] + 4;
                        var value = readList.Take(len).ToArray();
                        readList = readList.Skip(len).ToList();
                        ValueChanged(MsgType.BLEData, value);
                    }
                }
            }
        ...
        {
            while (readList[0] != 0xa5 &&(readList[0])!=0x5a)
            {
                readList.RemoveAt(0);
