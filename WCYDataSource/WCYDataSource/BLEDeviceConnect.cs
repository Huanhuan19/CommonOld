...
namespace WCYDataSource
{
    public class BLEDeviceConnect
    {
        ...
        private void Bluetooth_ValueChanged(MsgType type, byte[] pdata, BluetoothInfo btInfo)
        {
            ...
            {
                var NewPackage =ParseBuffer.Parse(1,pdata);
                if (NewPackage.IsNewCommandType)//新协议
                {
                    if (NewPackage.IsRightPackage)
                    {
                        switch (NewPackage.CommandType)
                        {
                            case NewCommandType.CommandType.GetDeviceInfo:
                                //SenserMsg sm = new SenserMsg() { index = index, sensorID = NewPackage.sensorID, connectName = BLEName, bluetooth = bt };
                                //OnAddSensorDataChanged(sm);//新协议屏蔽 ID不再是第二个
                                break;
                            case NewCommandType.CommandType.StartSendData:
                                byte sensorID = Convert.ToByte(BLEAdresse.Substring (4,3));
                                SenserMsg sm1 = new SenserMsg() { index = index, sensorID = sensorID, connectName = BLEName, bluetooth = bt };
                                OnAddSensorDataChanged(sm1);//新协议  ID藏在名字里了

                                DeceiveDataArgs dda = new DeceiveDataArgs() { reportBuff = pdata };
                                OnDeceiveDataChanged(dda);
                                break;
                        }
                    }
                }
                ...
                {
                    SenserMsg sm = new SenserMsg() { index = index, sensorID = pdata[1], connectName = BLEName, bluetooth = bt };
                    OnAddSensorDataChanged(sm);//新协议屏蔽 ID不再是第二个
            ...
            {
                //var buffer = Commands.SetCalibrationAndBatteryLevel(2);
                //SendCommand(buffer);
                //var buffer2 = Commands.SetUsbAndBluetoothConnectSymbol(1);
                //bluetooth.Write(buffer2);
            }
